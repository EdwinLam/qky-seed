    ...
    avalon.ready(function () {
        var vm = avalon.define({
            ...
            selectRole: function (role) {
                ...
                var loginSource = util.getQueryString("loginSource") || "";
                if(redirectUrl!=""){
                    redirectUrl = encodeURIComponent(redirectUrl);
                }
                if(loginSource!=""){
                    loginSource = encodeURIComponent(loginSource);
                }
                var url = "/sys/user/ssoLogin?userType=" + role.userType + "&uid=" + role.uid + "&schoolCode=" + role.schoolCode
                    + "&agencyCode=" + (role.agencyCode || "") + "&redirectUrl=" +redirectUrl+ "&loginSource=" +loginSource;
